...
from rsm.datasets.ecg_dataset import ECGDataset


class ECGSequenceDataset(ECGDataset):  # pylint: disable=W0223
  """Sequence generator for the ECG Dataset."""
  ...
  def get_train(self, preprocess=False, options=None):  # pylint: disable=W0221
    ...
    return self._dataset('train', 'training_data.npy', 'label_data.npy', preprocess, options)
  ...
  def _get_images_and_labels(self, images_file, labels_file):
    ...
    dirpath = os.path.join(self._directory, self.name)
    filepath = os.path.join(dirpath, images_file)

    images = np.load(filepath)
    images = np.reshape(images, [-1, 12, 1, 1])
    images = images.astype(np.float32)

    # Read and process the labels
    filepath = os.path.join(dirpath, labels_file)
  ...
  def _dataset(self, split, images_file, labels_file, preprocess, options):  # pylint: disable=W0613, W0221
    ...
    return tf.data.Dataset.from_generator(sequence_generator, output_types=(tf.float32, tf.int32),
                                          output_shapes=(tf.TensorShape([self.IMAGE_DIM, 1, 1]),
                                                          tf.TensorShape([])))
